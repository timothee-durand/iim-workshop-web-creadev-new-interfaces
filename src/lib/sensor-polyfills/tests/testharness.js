...
;(function () {
	...
	function is_service_worker(worker) {
		...
		return (
			'ServiceWorker' in self && Object.prototype.toString.call(worker) == '[object ServiceWorker]'
		)
	}
	...
	function assert_class_string(object, class_string, description) {
		assert_equals({}.toString.call(object), '[object ' + class_string + ']', description)
	}
	...
	function assert_throws(code, func, description) {
		try {
			func.call(this)
			assert(false, 'assert_throws', description, '${func} did not throw', { func: func })
		} catch (e) {
			...
			if (typeof code === 'object') {
				assert(
					...
					{ func: func, actual: e, actual_name: e.name, expected: code, expected_name: code.name }
				)
				return
			}
	...
	function Test(name, properties) {
		...
		this.phase = tests.phase === tests.phases.ABORTED ? this.phases.COMPLETE : this.phases.INITIAL
	...
	Output.prototype.setup = function (properties) {
		...
		this.enabled =
			this.enabled && (properties.hasOwnProperty('output') ? properties.output : settings.output)
	}
			...
			function () {
				...
				var rv = [
					[
						'section',
						{},
						['p', {}, 'Harness status: ', ['span', { class: status_class(status) }, status]]
					]
				]
