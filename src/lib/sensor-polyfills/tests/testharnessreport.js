...
var metadata_generator = {
	...
	jsonifyObject: function (objectValue, indent) {
		...
		if (1 == count) {
			for (var property in objectValue) {
				output += ' "' + property + '": ' + JSON.stringify(objectValue[property]) + ' '
			}
		} else {
			var first = true
			for (var property in objectValue) {
				...
				if (Array.isArray(value)) {
					output += this.jsonifyArray(
						value,
						indent + '                '.substr(0, 5 + property.length)
					)
			...
			if (this.cachedMetadata) {
				...
				for (var meta in testMetadata) {
					if (testMetadata.hasOwnProperty(meta)) {
						message = 'Single tests should not have metadata. ' + 'Move metadata to <head>. '
						break
					}
				}
			}
